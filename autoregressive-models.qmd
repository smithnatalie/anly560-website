...
mae1 <- matrix(NA,n-k,52)
mae2 <- matrix(NA,n-k,52)

st = tsp(flu.ts)[1]+(k-2)/52

for(i in 1:(n-k))
{
  #xtrain <- window(a10, start=st+(i-k+1)/12, end=st+i/12)
  xtrain <- window(flu.ts, end=st + i/52)
  xtest <- window(flu.ts, start=st + (i+1)/52, end=st + (i+52)/52)
  
  fit <- Arima(xtrain, order=c(2,1,1), seasonal=list(order=c(0,1,1), period=52),
                lambda=0, method="ML")
  ...
  # mae2[i,1:length(xtest)] <- abs(fcast2$mean-xtest)
  
}

plot(1:52, colMeans(mae1,na.rm=TRUE), type="l", col=2, xlab="horizon", ylab="MAE")
# lines(1:12, colMeans(mae2,na.rm=TRUE), type="l",col=3)
legend("topleft",legend=c("M1"),col=2:4,lty=1)

```

```{r}
print("1 step ahead mean error")
mean(err1)
```

```{r}
print("s steps ahead mean error")
mean(err2)
```
